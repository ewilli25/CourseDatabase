...
app.get("/courseInput", (request, response) => {
  ...
  app.post("/processCourse", async (request, response) => {
    let {studentname, name, gpa, grade, semester, professor, description} = request.body;

    let formattedGPA = Number(gpa).toFixed(2);
    let applicant = {StudentName: studentname, Name: name, GPA : formattedGPA, Grade : grade, Semester : semester, Professor : professor, CourseDescription : description};

    try {
        await client.connect();
       
        predictedAge = await getAge(studentname);
  ...
  app.post("/processReviewCourse", async (request, response) => {
    ...
    try {
        ...
        predictedAge = await getAge(result.StudentName);
